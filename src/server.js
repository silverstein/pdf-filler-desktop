...
app.post('/api/fill-local', async (req, res) => {
  try {
    ...
    if (fillData && typeof fillData === 'object') {
      try {
        const pdfDoc = await pdfService.fillFormFields(filePath, fillData, password);
...
app.post('/api/read-fields-local', async (req, res) => {
  try {
    ...
    const fields = await pdfService.readFormFields(filePath, password);
    res.json({ fields });
  } catch (error) {
...
app.post('/api/fill-pdf-local', async (req, res) => {
  try {
    ...
    const pdfDoc = await pdfService.fillFormFields(filePath, data, password);
...
app.post('/api/validate-pdf-local', async (req, res) => {
  try {
    ...
    const validation = await pdfService.validateForm(filePath, requiredFields, password);
    res.json(validation);
  } catch (error) {
...
app.post('/api/extract-text-local', async (req, res) => {
  try {
    const { filePath, password } = req.body;
    ...
    const textData = await pdfService.extractFullText(filePath, password);
    res.json(textData);
  } catch (error) {
...
app.post('/api/profiles/merge', async (req, res) => {
  try {
    const { profile1Name, profile2Name, newProfileName, mergeOptions = {} } = req.body;
    
    if (!profile1Name || !profile2Name || !newProfileName) {
      return res.status(400).json({ 
        error: 'profile1Name, profile2Name, and newProfileName are required' 
      });
    }
    ...
    res.status(201).json(mergedProfile);
  } catch (error) {
...
// Export profile
app.get('/api/profiles/:name/export', async (req, res) => {
  try {
    const { name } = req.params;
...
// Import profile
app.post('/api/profiles/import', upload.single('profileFile'), async (req, res) => {
  try {
    if (!req.file) {
...
app.post('/api/fill-with-profile-local', async (req, res) => {
  try {
...
app.post('/api/bulk-fill-local', async (req, res) => {
  try {
    ...
    const summary = {
      ...
      averageTimePerFile: `${(totalTime / csvData.length).toFixed(0)} ms`,
    ...
    console.log(`   Total time: ${summary.processingTime}`);
...
app.listen(PORT, () => {
  ...
  console.log('  - POST /api/profiles/merge            - Merge two profiles');
